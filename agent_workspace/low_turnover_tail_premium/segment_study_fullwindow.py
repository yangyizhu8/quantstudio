...
DB = r"D:/miniQMT策略实盘/QuantStudio/data/quantstudio.db.bak.balance_refill"  # 备用库（主库被新一轮 mcp_stock_minutes 锁; 主库核对已证两库逐位一致，研究附件等价）
OUT = Path(r"D:/miniQMT策略实盘/QuantStudio/agent_workspace/low_turnover_tail_premium/segment_study_fullwindow.json")
START, END = "2018-01-02", "2026-07-31"
TAIL = 0.10
BANDS = [("tail10", 0.0, 0.10), ("tail05", 0.0, 0.05), ("tail20", 0.0, 0.20), ("mid", 0.45, 0.55), ("high", 0.90, 1.00)]
...
_ms_start = int(pd.Timestamp("2018-01-01", tz="Asia/Shanghai").timestamp() * 1000)
...
for tag, lo, hi in BANDS:
    ...
    # 年度分解（风格归因：超额结构是否随市场风格切换）
    yearly = {}
    for (ym, _rd), e in zip(months, ex.tolist()):
        y = ym[:4]
        yearly.setdefault(y, []).append(e)
    yearly_out = {y: {"mean": round(float(np.mean(v)), 5), "n": len(v)} for y, v in sorted(yearly.items())}
    report["bands"].append({
        ...
        "yearly_decomposition": yearly_out,
    })
