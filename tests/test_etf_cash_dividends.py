...
覆盖：
  1. 公募免税口径：div_cash × volume 全额入账（不乘股票 0.80 短持税系数）
  2. reference 无 get_etf_dividends 方法 → no-op
  3. 空记录 → no-op
  4. 510500 除息日端到端：现金入账 = volume × div_cash，净值连续（非分红缺口）
...
# =====================================================================
# 1. 公募免税口径：全额入账（不乘 0.80）
# =====================================================================

def test_etf_cash_dividend_full_credit_exempt(engine):
    """510500 div_cash=0.149、持仓 10000 份 → cash += 1490.0（全额，非 1490×0.80）。"""
    ...
    assert engine.account.cash == pytest.approx(init_cash + 1490.0)
    ...
    assert evt["tax_policy"] == "etf_exempt"   # 公募免税，与股票 pre_tax_x_0.80 区分
    # 反证：不得套用股票 20% 短持税
    assert evt["cash_credit_net"] != pytest.approx(1490.0 * 0.80)
...
def test_510500_ex_date_nav_continuity(build_db):
    """510500 除息日（07-15，div_cash=0.149）：现金入账 = volume×0.149，
    净值变化 ≈ 0（价格缺口由现金补齐，非 -2.5% 缺口）。"""
    ...
    assert len(evts) == 1
    assert evts[0]["cash_credit_net"] == pytest.approx(10000 * 0.149)
    # 净值连续：现金入账补齐除息缺口（理论变化 = 0，允许佣金/过户费微量）
    chg = nav["2026-07-15"] / nav["2026-07-14"] - 1.0
    assert abs(chg) < 0.001, f"除息日净值缺口: {chg:.4%}"
    # 反证：若无入账，净值应 -2.48%（5.851/6.0-1）
    assert chg > -0.01
...
def test_same_day_dividend_and_split_not_mutually_exclusive(engine):
    ...
    assert engine.account.cash == pytest.approx(init_cash + 30100 * 0.05)
