...
class BacktestResultWindow(QMainWindow):
    def __init__(self, backtest_dir, root_path=None):
        ...
        self.root_path = root_path  # 用于 K线图的 db_path
        
        # 检测屏幕分辨率并设置字体缩放比例
        self.font_scale = self.detect_screen_resolution()
        
        # 无风险收益率（硬编码，原 QSettings 已移除）
        self.risk_free_rate = 0.03
        self.ptrade_metrics_summary = {}
        self.ptrade_round_trips = []
    ...
    def init_ui(self):
        ...
        for i, item in enumerate(col1_items):
            ...
            label.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Preferred)  # 标签宽度固定，高度自适应
            ...
            value.setStyleSheet(f"""
                ...
                background-color: transparent;
            """)
            value.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)  # 数值宽度自适应内容
            value.setAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
            if item == "回测区间":
                value.setWordWrap(False)
                value.setMinimumWidth(int(240 * self.font_scale))
            else:
                value.setWordWrap(False)  # 禁用自动换行
            
            info_layout.addWidget(label, i, 0, Qt.AlignmentFlag.AlignRight)
            info_layout.addWidget(value, i, 1, Qt.AlignmentFlag.AlignLeft)
        ...
        for i, item in enumerate(col2_items):
            ...
            info_layout.addWidget(label, i, 3, Qt.AlignmentFlag.AlignRight)
            info_layout.addWidget(value, i, 4, Qt.AlignmentFlag.AlignLeft)
            self.info_labels[item] = value
        
        info_group.setLayout(info_layout)
        # 设置基本信息面板的大小策略，限制其宽度
        info_group.setSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Preferred)
        info_group.setMaximumWidth(int(520 * self.font_scale))  # 放宽最大宽度，避免回测区间被截断
        ...
        chart_group.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        ...
        self.trades_table.cellDoubleClicked.connect(self._show_kline)
        ...
        self.daily_stats_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
    ...
    def load_data(self):
        ...
        try:
            ...
            ptrade_metrics_path = os.path.join(backtest_dir, "ptrade_metrics.json")
            self.ptrade_metrics_summary = {}
            self.ptrade_round_trips = []
            if os.path.exists(ptrade_metrics_path):
                try:
                    with open(ptrade_metrics_path, 'r', encoding='utf-8') as f:
                        payload = json.load(f)
                    self.ptrade_metrics_summary = payload.get('summary', {}) or {}
                    self.ptrade_round_trips = payload.get('round_trips', []) or []
                    print(f"已加载统一指标文件: {ptrade_metrics_path}")
                except Exception as e:
                    print(f"读取 ptrade_metrics.json 失败: {str(e)}")
            ...
            # 保存数据供 update_basic_info / _show_kline / 图表共用
            self.config_df = config_df.copy() if config_df is not None else None
            self.trades_df = trades_df.copy() if trades_df is not None else None
            self.daily_stats_df = daily_stats_df.copy() if daily_stats_df is not None else None
    ...
    def _set_metric_label(self, label_name, value_text, raw_value=None):
        """统一设置指标显示；数值类尽量复用现有正负着色逻辑。"""
        label = self.info_labels.get(label_name)
        if label is None:
            return
        if raw_value is None:
            label.setText(value_text)
        else:
            self.set_value_color(label, value_text, raw_value)

    def update_basic_info(self, config, daily_stats_df):
        """更新基本信息面板。优先使用 ptrade_metrics.json，确保 GUI / CLI 指标口径一致。"""
        try:
            strategy_name = os.path.splitext(os.path.basename(config['strategy_file']))[0]
            metrics = self.ptrade_metrics_summary or {}
            if metrics.get('strategy_name'):
                strategy_name = str(metrics.get('strategy_name'))
            ...
            if metrics.get('start_date') and metrics.get('end_date'):
                display_start = str(metrics.get('start_date'))
                display_end = str(metrics.get('end_date'))
            elif len(daily_stats_df) > 0:
                display_start = pd.to_datetime(daily_stats_df['date'].iloc[0]).strftime('%Y-%m-%d')
                display_end = pd.to_datetime(daily_stats_df['date'].iloc[-1]).strftime('%Y-%m-%d')
            else:
                display_start, display_end = start_time, end_time
            date_range_text = f"{display_start}\n{display_end}"
            self.info_labels["回测区间"].setText(date_range_text)
            self.info_labels["回测区间"].setToolTip(f"{display_start} ~ {display_end}")

            init_capital = float(config['init_capital'])
            self.info_labels["初始资金"].setText(f"{init_capital:,.2f}")

            if len(daily_stats_df) > 0:
                final_capital = float(daily_stats_df['total_asset'].iloc[-1])
            ...
            if metrics:
                self._set_metric_label("总收益率", f"{float(metrics.get('strategy_return_pct', 0.0)):+.2f}%", float(metrics.get('strategy_return_pct', 0.0)))
                self._set_metric_label("年化收益率", f"{float(metrics.get('annual_return_pct', 0.0)):+.2f}%", float(metrics.get('annual_return_pct', 0.0)))
                self.info_labels["最大回撤"].setText(f"{float(metrics.get('max_drawdown_pct', 0.0)):.2f}%")
                self._set_metric_label("夏普比率", f"{float(metrics.get('sharpe_ratio', 0.0)):+.2f}", float(metrics.get('sharpe_ratio', 0.0)))
                self._set_metric_label("索提诺比率", f"{float(metrics.get('sortino_ratio', 0.0)):+.2f}", float(metrics.get('sortino_ratio', 0.0)))
                self._set_metric_label("阿尔法", f"{float(metrics.get('alpha_ratio', 0.0)):+.4f}", float(metrics.get('alpha_ratio', 0.0)))
                self.info_labels["贝塔"].setText(f"{float(metrics.get('beta_ratio', 0.0)):+.4f}")
                self._set_metric_label("基准收益率", f"{float(metrics.get('benchmark_return_pct', 0.0)):+.2f}%", float(metrics.get('benchmark_return_pct', 0.0)))
                self._set_metric_label("基准年化收益率", f"{float(metrics.get('benchmark_annual_return_pct', 0.0)):+.2f}%", float(metrics.get('benchmark_annual_return_pct', 0.0)))
                self.info_labels["胜率"].setText(f"{float(metrics.get('win_rate_pct', 0.0)):.2f}%")
                self.info_labels["盈亏比"].setText(f"{float(metrics.get('profit_loss_ratio_pct', 0.0)) / 100:.2f}")
            else:
                total_return = (final_capital - init_capital) / init_capital * 100 if init_capital > 0 else 0.0
                self._set_metric_label("总收益率", f"{total_return:+.2f}%", total_return)
                self._set_metric_label("年化收益率", "0.00%", 0.0)
                self.info_labels["最大回撤"].setText("0.00%")
                self._set_metric_label("夏普比率", "0.00", 0.0)
                self._set_metric_label("索提诺比率", "0.00", 0.0)
                self._set_metric_label("阿尔法", "0.0000", 0.0)
                self.info_labels["贝塔"].setText("0.0000")
                self._set_metric_label("基准收益率", "0.00%", 0.0)
                self._set_metric_label("基准年化收益率", "0.00%", 0.0)
                self.info_labels["胜率"].setText("0.00%")
                self.info_labels["盈亏比"].setText("0.00")

            # 交易相关指标：优先使用统一指标的 round-trip / summary；其余仍保留旧逻辑兜底。
            trade_days = int(metrics.get('trade_days', len(daily_stats_df))) if metrics else len(daily_stats_df)
            trade_count = len(self.ptrade_round_trips) if self.ptrade_round_trips else max(0, len(self.trades_df) // 2) if getattr(self, 'trades_df', None) is not None else 0
            daily_trades = trade_count / trade_days if trade_days > 0 else 0.0
            self.info_labels["日均交易次数"].setText(f"{daily_trades:.2f}")

            if self.ptrade_round_trips:
                round_trips_df = pd.DataFrame(self.ptrade_round_trips)
                win_trips = round_trips_df[round_trips_df['pnl'] > 0] if 'pnl' in round_trips_df.columns else pd.DataFrame()
                loss_trips = round_trips_df[round_trips_df['pnl'] < 0] if 'pnl' in round_trips_df.columns else pd.DataFrame()
                max_profit = float(win_trips['pnl'].max()) if len(win_trips) else 0.0
                max_loss = float(abs(loss_trips['pnl'].min())) if len(loss_trips) else 0.0
            ...
            if getattr(self, 'trades_df', None) is not None and len(self.trades_df) > 0 and len(daily_stats_df) > 0:
                try:
                    daily_trades_calc, max_win_streak, max_loss_streak, _, _ = self.calculate_trading_metrics(self.trades_df.copy(), daily_stats_df)
            ...
            if len(daily_stats_df) > 0 and 'daily_return' in daily_stats_df.columns:
                volatility = self.calculate_volatility(daily_stats_df['daily_return'])
                self.info_labels["年化波动率"].setText(f"{volatility * 100:.2f}%")
            else:
    ...
    def update_trades_table(self, trades_df):
        ...
        try:
            ...
            if len(trades_df) > 0:
                ...
                header.setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)  # 交易时间
                header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # 证券代码
                header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # 交易方向
                header.setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)  # 成交价格
                header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # 成交数量
                header.setSectionResizeMode(5, QHeaderView.ResizeMode.Stretch)          # 成交金额
                header.setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents)  # 手续费
                ...
                self.trades_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
    ...
    def _show_kline(self, row, col):
        """双击交易记录行 → 弹出该标的的 K线图 + 买卖标记"""
        try:
            code_item = self.trades_table.item(row, 1)  # column 1 = 证券代码
            if not code_item:
                return
            code = code_item.text()
            ...
            if self.trades_df is not None and len(self.trades_df) > 0:
                mask = self.trades_df['code'].astype(str).str.contains(bare, na=False)
                trades = self.trades_df[mask].copy()
                # 还原列名（load_data 做了 time/direction 重命名，KlineViewer 期望 datetime/action）
                trades = trades.rename(columns={'time': 'datetime', 'direction': 'action'})
                trades['action'] = trades['action'].map({'买入': 'buy', '卖出': 'sell'}).fillna(trades['action'])
            else:
                trades = None

            start_date = ""
            end_date = ""
            if self.daily_stats_df is not None and len(self.daily_stats_df) > 0:
                start_date = str(self.daily_stats_df['date'].iloc[0])[:10]
                end_date = str(self.daily_stats_df['date'].iloc[-1])[:10]
            ...
            self.kline_win = KlineViewer(db_path, bare, trades, start_date, end_date)
            self.kline_win.show()
        except Exception as e:
            print(f"打开K线图失败: {e}")
    ...
    def update_daily_stats_table(self, daily_stats_df):
        ...
        try:
            ...
            if len(daily_stats_df) > 0:
                ...
                header.setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)          # 总资产
                header.setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)          # 持仓市值
                header.setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)          # 可用资金
                header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # 日收益率
                ...
                for i, row in daily_stats_df.iterrows():
                    ...
                    total_asset_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                    self.daily_stats_table.setItem(i, 1, total_asset_item)
                    
                    # 持仓市值
                    market_value_item = QTableWidgetItem(f"{row['market_value']:,.2f}")
                    market_value_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
                    ...
                    if 'daily_return' in row:
                        ...
                        daily_return_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        try:
            ...
            if os.path.exists(daily_stats_path):
                ...
                if len(daily_stats_df) > 0 and 'date' in daily_stats_df.columns:
                    ...
                    # 获取交易日天数
                    n = len(pd.bdate_range(first_date, last_date))
