...
def _ensure_runtime_state():
    ...
    if not hasattr(g, 'current_holdings'):
        g.current_holdings = {}  # 策略自维护当前持仓集合（有序 dict 兼作集合，确定性容器规范）
...
def handle_data(context, data):
    ...
    # ---- 构造卖/买指令 ----
    target_keys = dict.fromkeys(target_codes)
    # 卖出：退出信号 + 不在新目标名单（确定性：sorted 输出，禁止哈希序迭代）
    sell_keys = {}
    for c in exit_codes:
        sell_keys[c] = 1
    for c in holding_codes:
        if c not in target_keys:
            sell_keys[c] = 1
    sell_codes = sorted(sell_keys)
    # 买入：目标名单中当前未持有（以自维护持仓为准），且未满 TOP_N
    held_after_sell = [c for c in holding_codes if c not in sell_keys]
    ...
    for code in sell_codes:
        ...
        g.current_holdings.pop(code, None)
