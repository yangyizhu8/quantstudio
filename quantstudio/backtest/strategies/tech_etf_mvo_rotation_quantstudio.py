...
- 周频：run_daily 9:31 调度，ISO 周边界去重，仅每周第一个交易日调仓；
- 暖机期（历史不足 252 日）持币空仓；缺数据标的 fail-soft 跳过。

平台纪律：
- 零 import；np/pd/g/log 由引擎注入；不使用 MyTT/get_current_data/check_limit/data[code]；
...
def _get_close(code, count):
    ...
    try:
        ...
        log.warn('get_history 失败 %s: %s' % (code, e))
        return None
    try:
        if hist is None:
            return None
...
def weekly_rebalance(context):
    """每日 9:31 被调度；ISO 周变化时执行评分+MVO+风控+调仓。"""
    _ensure_runtime_state()
    ...
    for key in held_keys:
        if _bare(key) not in target_bares:
            ...
            if amount > 0:
                ...
                if oid is None:
                    log.warn('[%s] 清仓委托失败: %s' % (STRATEGY_ID, key))
                else:
    ...
    for code, w in weights.items():
        ...
        if oid is None:
            log.warn('[%s] 委托失败: %s -> %.0f' % (STRATEGY_ID, code, target_value))
        else:
