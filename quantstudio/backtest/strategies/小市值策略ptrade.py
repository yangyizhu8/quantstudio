...
def get_trade_stocks(context, data):
    # 获取持仓中涨停的标的
    # 注意：get_positions()、get_fundamentals() 和 context.portfolio.positions
    # 均按真实 PTrade 策略容器语义使用 .SS/.SZ。持仓涨停股必须保持原格式并入
    # buy_stocks，避免手工转成 .XSHG/.XSHE 后与精确 dict membership 不一致。
    hold_up_limit_stock = [stock for stock in g.pre_position_list if check_limit(stock)[stock] == 1]
    df = g.df
    if df.empty:
