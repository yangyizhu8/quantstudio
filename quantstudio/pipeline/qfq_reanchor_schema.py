...
DDL_DUCKDB: Dict[str, str] = {
    # ---- anchor 状态（v3 §5.1 + v4 §5 last_stale_probe_at + v5 §5.2 probe 失败列）----
    "qfq_anchor_state": """
    ...
    # ---- 重锚事件（v3 §5.2 + v4 §7 event_type + v5 §5.1 故障类聚合三列 + cycle_business_date）----
    "qfq_reanchor_event": """
    ...
    # ---- bootstrap 运行（v3 §4.3b）----
    "qfq_bootstrap_run": """
        CREATE TABLE IF NOT EXISTS qfq_bootstrap_run (
            bootstrap_run_id VARCHAR PRIMARY KEY,
            asset_type       VARCHAR,
            params           VARCHAR,
...
DUCKDB_COLS: Dict[str, List[str]] = {
    "qfq_anchor_state": [
        "asset_type", "code", "price_source", "detection_source",
        "detection_anchor_factor", "factor_date", "anchor_version", "status",
        "locked_detection_factor", "locked_price_anchor_version", "last_ex_date",
        "last_event_id", "retry_count", "last_stale_probe_at",
        "last_stale_probe_error", "probe_fail_count", "updated_at",
    ],
    "qfq_reanchor_event": [
        "event_id", "event_type", "asset_type", "code", "price_source",
        "detection_source", "old_factor", "new_factor", "old_factor_date",
        "new_factor_date", "daily_method", "minute_ratio_plan", "xtquant_price_ratio",
...
def init_duckdb_schema(conn) -> None:
    """在给定 DuckDB 连接上创建全部 QFQ 重锚主库表（幂等）。

    事务感知：本函数**不 commit / 不开新事务**，只发 DDL。调用方决定事务边界
    （standalone init 走 init_all_from_paths；编排内可并入现有事务）。
    对存量库执行缺失列自动 ALTER（幂等）。
    """
    for ddl in DDL_DUCKDB.values():
        conn.execute(ddl)
    _migrate_duckdb_columns(conn)
...
def _migrate_duckdb_columns(conn, best_effort: bool = False) -> None:
    ...
    新表由 CREATE TABLE IF NOT EXISTS 建全，不触发 ALTER；仅当早期开发库已有旧结构表
    缺新增列时补齐。列已存在则跳过。

    可靠性（阻断 5 修复）：默认 **fail-fast**——DESCRIBE 失败或 ALTER 失败都抛异常，
    不让缺列错误延迟到正式修正事务。仅在显式 ``best_effort=True`` 时才降级为
    warning 并继续（可选兼容，不推荐生产路径使用）。
    """
...
# 正确性关键状态表的完整契约（从 DDL 解析，单一真相源）。
SCHEMA_CONTRACT_DUCKDB: Dict[str, Dict] = {
    t: _parse_ddl_contract(d) for t, d in DDL_DUCKDB.items()}
