...
| `get_fundamentals(security,table='valuation',fields,date,...)` | 财务/估值数据。返回 `DataFrame(index=code, columns=fields)`（P-D10 修复后统一契约）。`growth_ability` 等表存在本地→平台字段名映射（如 `or_yoy → operating_revenue_grow_rate`），转换器自动翻译，策略仍按本地字段名消费；缺失字段走 `QS_SHIM_FIELD_MISSING` 显性警报而非静默空。**eps 表保真映射（2026-08-24 P-A2，opt-in）**：设 `SourceConverter(fidelity_eps_basis='basic'/'diluted')` 后，转 PTrade 产物请求平台 `basic_eps`/`diluted_eps` 列（本地 eps 语义锚，探针实证双端数值一致），返回列逆翻译回 `eps`/`diluted_eps` 名，策略无感知；默认 `passthrough` 时产物逐字节不变。**eps 跨表回补（2026-08-25 P-A3）**：fin_indicator.eps 与 income_statement.basic_eps 为同源复制列（对账 98.1% 相等）；源端回填错位造成 eps NULL 时，写路径自动以同 (code,end_date) basic_eps 回补（ann_date 取两表较大者 PIT 保守；`backfill_eps_source` 打标列可审计；幂等可逆；无缺口库零行为）。门禁 `EpsBackfillGap` 每日检查防再发。**注意：回补是数据层行为，策略层仍禁止手工补救 NULL（见 3.5 authority-locked 规则）**。**平台可移植性 v8 系列（2026-08-31，框架层吸收，策略零改动）**：转 PTrade 产物经统一 wrapper（唯一 `get_fundamentals`，eps 常量烘焙防多模板覆盖）——`date+range` 双参数走平台原生 range 多期 + multi2 拍平 + publ_date PIT（未披露 NaN 占位期剔除）→ end_date 归一 **epoch 毫秒**（本地策略 `np.datetime64(int(end_date),'ms')` 消费契约）；平台 range 模式 publ_date 全空缺省容错；`report_types` 按 end_date 月日过滤；大池（>32）批量预取自动 SKIP（逐股回退，幂等）（v8.8 起 range 形态改 3 码/组分组预取 + B6c 缓存命中，≈2.4 倍提速，行为等价）；balance/income/valuation 平台缺列（total_share 等 8 字段）首调短路 + 全缺返回 1 行 NaN 契约（策略 NaN 比较恒 False 降级不误加分）。门禁：`scripts/run_contract_gate.py`（契约套件 + `check_fund_matrix --check` 哈希门）与契约矩阵 `docs/evidence/fundamentals-contract-matrix.yaml`（唯一事实源，缺口/RD 登记）。转换注入的公共 helper（`_qs_g_obj` 等）无条件注入（2026-09-01 鲁棒性修复：行业/金融池段不再依赖 fundamentals 段条件注入而悬空）。**2026-09-04 平台形态吸收（九轮复验定谳，证据 §11-18）**：`date=None` 白名单表前日 PIT 拼接（`QS_GF_DATE_SYNTH`）；list >500 码自递归分块（`_QS_GF_LIST_CHUNK`）；报表表未显式传参自动 range 5 年路由（`QS_GF_STATEMENT_RANGE`，平台 date 形态=披露时点单期语义）；range PIT `publ_date` epoch 毫秒数值优先归一 + `report_types` 报告期 +8h UTC 偏移；**valuation 列名运行时判型**（`QS_VAL_MODE` 探针，每回测一次）→ 平台模式启用 `pe_ratio↔pe_ttm`/`turnover_ratio↔turnover_rate`/`pb_ratio↔pb` 等直映（本地名列零触发），异常时换手率合成兜底（vol×close/float_value×100 + 量级自校准，`vhit/chit` 审计）。 |
