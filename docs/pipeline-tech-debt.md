...
## 云端 etf_minutes close 口径与客户端还原链自洽性疑问（2026-09-23 登记 · 低优先）

- **状态**：REGISTERED（低优先，与 qfq 域尾巴同池）→ 后续排期；本轮未闭环
- **来源**：客户反馈包（2026-09-23）问题3 修复过程中的实证，非客户原报问题
- **已实证事实**：
...
- **影响**：①被 UnitCheck 正确拦截的异常行会持续进隔离区（已由分表型门禁解除
  「批次判负」，见 commit `c62c1cc`）；②若还原乘数在部分码上偏大，主库 raw 价格
  可能被系统性放大——**当前无证据表明已污染主库**，但口径疑点未闭环。
- **解除条件**：①引入可独立核验的基准（外部行情源或云端 qfq 锚口径说明），
  对还原后 raw 与基准做逐值比照；②定位 `≈9×` 异常的码与日期区间（已转数据域跟进）。
- **推进归属**：单独排期，与本批四问题修复解耦；不阻塞本批推送。
