...
- ⚠️ **撮合/估值 raw 口径、信号前复权（2026-08-14 PTrade 实证对齐）**：成交价、持仓估值、`data[code].price` 均来自引擎原始价快照（raw OHLC，不复权）——PTrade 平台实证撮合价 = raw close（日线 5/5、分钟 6/6 精确）。信号取数保持前复权（`fq='pre'`）。两条链路分离：策略信号序列（`get_history(fq='pre')`）与撮合价（raw close）口径不同是**预期行为**，除权日两者差异即复权因子；比较成交记录与信号价时须注意口径差异。**ETF 除权日持仓由引擎自动补正（2026-08-16，引擎方案 v2-final）**：`stock_dividend` 无 ETF 记录，引擎 `_apply_factor_derived_split` 用 preClose 反推处理 ETF 送股/份额折算/合并（仅 ETF，股票零侵入），除权日净值不会虚假腰斩；**ETF 现金分红由引擎 `_apply_etf_cash_dividends` 按 etf_dividend 表税前 × 0.8 入账**（2026-08-16 PTrade 实测：平台对 ETF 与股票统一扣 20%——公募税法免税与平台实现不一致，回测以平台实测为准）；策略无需也不应在代码里自行做 ETF 送股/复权/分红补正。
...
- ⚠️ **平台差异由框架吸收，禁止手写兜底（2026-08-22）**：市价单单笔上限（创业板/科创板 50,000 股，超限整单取消）由转换管线自动拆单（>49,000 股拆多笔）；`current_price`/`get_current_data` **非真实 PTrade API**（官方文档全文核实 + 模块加载期 NameError，双证）——双端/PTrade 目标禁止调用（Validator `TARGET-LOCAL-EXTENSION-BAN` BLOCK），平台取现价用 `data[code].price`/`get_history` 最新 bar/`get_price`；`get_trade_days` 全量日历/格式混用由转换侧归一；`get_snapshot` 仅交易场景（回测不可用）。策略生成**禁止**自带 `_normalize_date_str`/`_current_raw_price`/`g.last_close` 等兜底（Validator `PTRADE-PLATFORM-FALLBACK-BAN` BLOCK）。**2026-09-03 追加吸收**：`get_Ashares(exclude_bse=…)` 的 `exclude_bse` 为本地扩展参数（平台仅接受 `date`）→ 转换管线剥离 kwarg 并按源语义烘焙 `_QS_EXCLUDE_BSE`；`set_commission` 平台要求佣金费率/最低佣金均 >0 → 常量 ≤0 吸收为可表达下限（min_commission 0.01 / commission_ratio 1e-6）、动态表达式包装 `max(expr,floor)`。校验器 BLOCK：`PORTABILITY-ASHARES-EXCLUDE_BSE` / `PORTABILITY-COMMISSION-MIN-ZERO` / `PORTABILITY-COMMISSION-POSITIONAL`（本地专用策略不受限）。详见 `docs/strategy-compiler/ptrade-platform-absorptions-design.md`。**2026-09-04 get_fundamentals 平台形态吸收（九轮平台复验定谳，设计/证据见 gf-date-synthesis-design.md + ptrade-platform-absorptions-20260903.md §11-18）**：① `date=None` 白名单财务表 → 前日 PIT 拼接（`QS_GF_DATE_SYNTH`，对齐本地 prev_date 语义）；② list 单调用码数上限 → ≤500 自递归分块（`_QS_GF_LIST_CHUNK`，P-D10 500 码实证/800 码空返实证）；③ 报表表（income/balance/cashflow）date 形态=披露时点单期语义 → 未显式传参时自动 range 路由 `[date年-5, date年]`（`QS_GF_STATEMENT_RANGE`，CAGR/ROE 类需多期）；④ publ_date epoch 毫秒数值优先归一 + report_types +8h UTC 偏移（两环境形态自适应）；⑤ **valuation 表列名双环境相反**（平台=聚源短名 pe_ttm/pb/ps/pcf/turnover_rate/total_shares，无 pe_ratio/turnover_ratio/pb_ratio）→ 运行时 `QS_VAL_MODE` 列名探针判型（每回测一次）才启用映射，本地名列零触发；turnover_ratio 直映 turnover_rate，异常时合成兜底（vol×close/float_value×100 + 中值量级自校准）；⑥ 配套：`validate_local_strategy` LOCAL-API-WHITELIST 全深度收集（嵌套 def/嵌套 import 不误 BLOCK）。策略生成仍**禁止**自带列名探测/日期拼接/分块逻辑（全部框架吸收）。
