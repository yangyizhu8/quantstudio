...
## 七、实施与验收结果（终稿，2026-09-20）

### 7.1 一处定性撤回（②「激活迁移路径」不成立）

实施取证发现 `_migrate_duckdb_columns` **不在生产 init 路径**（docstring :705 明示
"不再被普通 init 调用"；`test_qfq_schema_status.py:324/336` 以 **spy 证明**
COMPLETE_2_1 与 2.0 fail-fast 路径均不调）。故 §六·落定 1 的「激活既有迁移路径 /
属 schema 迁移生效」升格**撤回**；本件定性回到**真相源订正**。V5 相应降级为
**受控工具行为验证**（非生产迁移生效）。

### 7.2 验收实测（V1–V5 全 PASS）

| # | 项 | 实测 | 结果 |
|---|---|---|---|
| V1 | 真相源逐位相等 | 脚本断言 DDL 13 == COLS 13，逐位一致 | PASS |
| V2 | 两条契约测试 | 改前 `2 failed` → 改后 `2 passed`（定向测量） | PASS |
| V3 | 全量对表（基线 61） | `59 failed, 3009 passed`（底稿 docs/evidence/v3-full-202609191624.txt） | PASS |
| V4 | 消费点回归 | 随 V3 全量：消费面无失败 | PASS |
| V5 | 迁移双态（工具级） | 已建库 13→13 无变更(no-op)；缺列库 10→13 恰补三列 | PASS |

**V3 零新增论证（比计数更强）**：59 条失败按文件全谱归类，**因果面检索
（qfq_schema/reanchor/schema_status/duckdb_cols/column_order）为空集**——三行追加的
可观察面只有该表列集/列序校验，该家族失败为零 ⇒ 新失败无处可生；家族分布与基线 61
同源，差集恰为预测的两条。诚实边界：未做改前 61 条严格集合 diff（无改前完整清单），
以因果面空集 + V2 定向测量替代（总调度审定免除路径 1）。

### 7.3 过程自纠记录（三例，均自测打回）

1. 误定性"纯新库 UPDATE 必失败"（DDL 实已含三列）→ 撤回，改定 drift；
2. 误升格"激活迁移路径"（未先查生产调用链）→ 撤回，V5 降级工具级；
3. V5 测试台只建一表致 `DESCRIBE qfq_anchor_state` 失败 → 修台（建全表）不改产码。
