...
## 7. 现网校准抽样（B 案口径裁定 · 加固版）

按总调度 B 案裁定「**生产备份库为主口径 + 现网抽样校准一次**」，判据为
「现网与备份**同窗口配对**各跑，偏差须落在既有组内极差量级（约 2~5 s）内」，
超出则回报重裁。校准窗口 `2025-09-01 ~ 2026-09-03`（245 交易日），现网于空闲窗实测。

**加固过程**：初次配对（各 2 轮）偏差 **+4.900 s，余量仅 0.100 s（临界）**；
经总调度批准（「0.1 s 余量的临界结论日后必被追问，趁窗加固是最优解」），
于现网窗口关闭前追加至**各 4 轮**。

| 口径 | 序列（s） | **中位数** | 极差 | 均值 |
|---|---|---|---|---|
| **现网生产库** | 56.448 / 52.817 / 49.230 / 49.426 | **51.121** | 7.219 | 51.980 |
| **备份库 2026-09-12** | 47.036 / 52.428 / 45.799 / 53.314 | **49.732** | 7.516 | 49.644 |

**偏差（中位数）= +1.389 s（+2.79 %）**；阈值 **5.0 s**；**余量 3.611 s**
⇒ **判定成立：备份库口径数字可标注为「生产口径」。**

| 阶段 | 偏差 | 余量 |
|---|---|---|
| A 案（各 2 轮） | +4.900 s | **0.100 s（临界）** |
| **B 案（各 4 轮，本次）** | **+1.389 s** | **3.611 s** |

**方向合理性**：现网比备份慢 **+2.79 %**，与现网库大 13 GB、`stock_daily` 多 0.46 % 的
预期方向一致，**非异常**。行为核对：两侧 `nav_len` 均为 245、`hit/miss` 均为 246/0。

**判据纪律**：判读标准于校准前固定（B 案原文），**未事后解释**；本次加固系判定成立后
为消除临界性而追加，**不属于放宽阈值，亦不改判据**。

**原始读数**：`agent_workspace/cachekey_profiling/cal_{prod,shadow}_r{1..4}.json`
