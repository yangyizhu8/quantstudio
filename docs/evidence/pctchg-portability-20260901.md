...
## 6.14 第二十轮平台终验（2026-09-03 21:14 回执）——判据 1/2 通过、3/4 证伪待根因

**判据 1/2 通过**：版本行 `v=20260903-v10.2` + `QS_MINUTE_SYNTH` 四候选全 True
（data 捕获→合成链路机械生效）。

**判据 3/4 被权威日线证伪（审核独立取证，data_source=mcp 本地权威库）**：
000017 于 07-01 开 6.06/高 6.29/收 6.08，涨停价 6.73，全天未触板；001237 高 75.99 <
涨停价 78.89，同未触板。日线 high 是全天上限——任何分钟粒度不可能含 6.73，
"开盘秒板"解释为循环论证（撤回）。两笔买入为假阳性；07-02 对 001237 的 T+1 卖出是
错误建仓的止损，不构成正向旁证。600363/603983 未买仅因 MAX_HOLDINGS=2 截断
（候选=T-1 封板股，昨日 bar 判定恒真 → 四候选全部假阳性，100% 假阳性生成器）。

**根因机制重建（详见 `docs/evidence/v10-delta-notes-20260903.md`）**：平台回测
`data[code]` 提供昨日 bar（close=昨日封板价、preclose=前日收盘——与平台分钟
get_history include=False 返回昨日 bars 同源，平台回测数据面整体滞后一日）。
v10.2 将昨日 bar 合成为"当前 bar" → 假阳性。

**v10.3 修复**：日期守卫（bar 日期==current_dt 才合成；stale 拒绝 fail-open）+
三元组审计（SYNTH 行内嵌 close/preclose/t）。本地三场景复现（今日 bar 合成 True/
stale bar 拒绝/断板反包零捕获行）+ fund 复证（42 测试 → 哈希 d1c47919）+ gate PASS。
E2E 维持 BLOCKED(duckdb 锁竞争，挂起栈与 v9 对照一致)。待第二十一轮平台实证：
预期四候选 `=stale-bar` + 无买入订单（负向正确闭环）。

**第二十一轮回执（2026-09-03 22:14）——stale 假说被三元组推翻，真根因 = preclose=0.0 脏值**：
`000017.SZ=True[close=6.08 preclose=0.0 t=2026-07-01 09:31:00]`——t=今日 09:31（data 为
今日 bar，非 stale）且 close=6.08（今日盘中价，与日线收 6.08 吻合；001237=75.09 盘中价
≠ 收 73.17 佐证非日线快照）。**平台 BarData.preclose 返回 0.0 伪值** → 策略
prev_close=0.0（非 None，穿透 g.prev_close 回退）→ lp=0.0×1.1=0.0 → last_close≥0 恒真
→ 无条件假阳性（第二十轮两笔买入的真实机制，非 stale bar）。

**v10.4 修复**：合成行 **omit preClose**（不再写 data.bar.preclose）→ preClose 列由 v3
日线昨收合成统一填充（6.12 → lp=6.73）。双窗口态数学论证：平台日线窗口 [06-29,06-30]
（6.12→6.73）或含今日 [06-30,07-01]（6.08→6.69），四候选 last_close（6.08/75.09/36.7/
28.73）全部 < lp → 负向正确。本地复现：preclose=0.0 脏值场景 → 合成行 omit → v3 填充
→ trigger=False 双态 ✓ + fund 复证 + gate PASS。

**第二十二轮回执（2026-09-03 22:47）——负向正确闭环达成**：
`QS_MINUTE_SYNTH v=20260903-v10.4 四候选 True[close=今日盘中价 preclose=None t=今日]` +
**全程零订单 + 全程 positions=0**（07-02 无卖出——无错误建仓即无止损）。与权威日线完全
一致：四候选 07-01 全天未触板（high 6.29/75.99/38.25/30.76 < 涨停价 6.73/78.89/39.36/
30.95）→ 正确不买。**判据 3/4 以"与权威行情一致"重新成立**——假阳性生成器（preclose=0.0
路径）切断，打板判定基准恢复正确。

## 6.15 本地参照对照与正向存活探针（2026-09-04 第二十三轮审核要求②闭环）

**方法**（引擎 duckdb 写锁 BLOCKED 的等价绕行）：exec v10.4 产物（代码零改动）+ mock 层读
本地权威库（平台语义忠实复刻：分钟 stale=T-1 尾盘、preclose=0.0 脏值、日线止于 T-1、
include=True 可见当日 close），07-01~07-10 全窗逐日驱动。

- **筛选同构**：candidates 逐日双端一致——07-01 **四码逐码一致**（000017.SZ/001237.SZ/
  600363.SS/603983.SS ≡ 平台 SYNTH 行），07-02~10 双端均 0；五统计 mkt_zt/ladder 全符
  （±1 内=池边界差），promo/zp ≤0.02。**候选池逐日刷新（4→0→…）非恒 4 码——审核阻断情形排除。**
- **归因**：07-01 真封板 6 只逐条件重放：000004 为升首板（T-1 非二板，正确不入池）；
  001237 类恰二连板但**量能维度（vol_ge2>1.2/vol_5d≥1.5×avg5）合法落选**（002808/002407
  vol_5d 未过；600113/603950/600378 vol_ge2 未过）——零订单=「二进三+放量」筛选语义的
  合法结果，**非平台假阴性**。
- **正向存活探针**：注入 09:31 bar 000017 触板价 6.73 → **BUY 8200 股生成**（与平台第二十轮
  真实订单股数逐位吻合，px_exec=日线当日 close 6.08 口径同源）——合成→lp 判定→拆单→下单
  全链活着。
- **定谳**：判据 4 补全——零订单与本地参照完全一致（同 4 码、零买），正向路径本地实证存活；
  平台正向证据待真实封板日窗口回补（登记待补证，不阻断推送）。引擎 E2E 维持
  F-DUCKDB-LOCK/F-LOCAL-MIN 独立框架问题登记（不销项，本 parity 为其等价证据）。

## 9. 推送门状态（v10 系列：待用户确认推送）

- v10.0-v10.2：两轮实施 bug（SYNTH 打点 UnboundLocalError / 合成参数序颠倒）+ 一轮
  假阳性（preclose=0.0）——全部本地复现→修复→归档（`v10-delta-notes-20260903.md`）。
- v10.3-v10.4：日期守卫 + omit preClose + 三元组审计——第二十二轮平台实证负向正确闭环。
- 验收链：mock 四场景 / fund_matrix 复证 42 测试 ×4 / CONTRACT GATE PASS ×4 / 权威日线
  事实核验（第二十轮证伪 + 第二十二轮闭环）。
- 待办：E2E 补跑（duckdb 锁竞争 BLOCKED 维持）；测试债 7 个 test_source_import 断言
  （下一工作周期）。
- **Step 5 状态：验收证据齐备，待用户确认后执行 Step 6 双仓推送**（清单：source_import.py、
  两产物、fund_matrix YAML/MD、设计/证据文档 ×3、README/toolbox/prompt_engineering）。
