...
> **生产零接触判据澄清（2026-09-16 追加）**：本文"生产库 LastWriteTime"仅为本轮当时的**辅助观察**，
> **不可作为长期判据**——生产库由数据线任务正常写入（实测：2026-09-16 06:37:57 `etf_minutes`、
> 07:30:11 `stock_minutes` 更新，主库 +2.17 GB；`data/logs/daemon.log` 同期未更新、无 daemon 进程，
> 即非 `daemon.py` 采集链、**非本轮操作**）。
> **本轮的权威零接触证据是「目标隔离」**：`QUANTSTUDIO_DATA_ROOT` 影子化后六条关键路径**全部落在影子根**（§1 表），
> 且影子库实际接收了本轮真实拉取写入（`etf_basic aligned=1606 → written 1606 rows`，§4）——
> 二者共同证明本轮写入目标与生产库物理隔离。**后人复核 A6 时请以此为准，勿以 mtime 判定。**
