...
## 七、问题 3 处置：分表型质量门禁（用户裁定选项1，已实施）

用户 2026-09-23 裁定：**选项1 —— 分钟表独立阈值、日线保持 1%、只改批次判负线不改行级判拒**；
阈值取值须附实证依据（选项2 本期否决；选项3 的云端发现转数据域跟进）。

### 阈值推导（40 个 1 日窗实测，非拍值）

`scripts/acceptance/derive_minute_gate_threshold.py`，2025-01~2026-09 各 20 窗：

| 表 | P50 | P90 | P99 | max | 行级聚合 |
|---|---|---|---|---|---|
| etf_minutes | 4.3755% | 4.9338% | **5.0499%** | 5.0635% | 3.8940%（3,088,106 行） |
| stock_minutes | 0.1539% | 1.9978% | **2.0273%** | 2.0320% | 0.6636%（4,973,195 行） |

**阈值 = max(P99) × 1.58 ≈ 8%**（整数、单一值覆盖两表、对最大实测窗余量 58%）。
同批次另有实测点：客户判负记录 1.4985%（etf 全窗）；既有文档 1.49% 触发判负、
stock_minutes 回补 0.108% 未触发。**真单位错（手/股 ×100）时拒绝率 = 100%**，
故 8% 距其仍余 12.5 倍 ⇒ 门禁不被架空。

> 附注：取证同时发现 **2026-06 之后云端数据质量显著改善**（etf 4.11%→0.80%→0%；
> stock 0.06%→0%），即历史高拒绝率部分源于已退出的数据质量区间；阈值按历史
> 最坏区间取值，属保守选择。
...
| 文件 | 改动 |
|---|---|
| `daemon.py:60-63` | 新增 `_MINUTE_FREQS`（1/5/15/30/60min） |
| `daemon.py:3247-3303` | `_failure_gate` 增 `freq` 参数 + 分钟阈值分支（配置键 `max_reject_rate_minute`，默认 `0.08`） |
| `daemon.py` ×4 处 | 拒绝率调用点补 `freq=freq`（普通/流式/两处逐日路径） |
| `tests/test_quality_gate_by_freq.py`（新） | 9 例回归钉 |

**开发期自纠**：初版把分钟阈值嵌在 `if is_reject and source == "mcp"` 分支内 ⇒
xtquant 分钟表（历史手/股单位错配事故路径）漏配、阈值仍 0.01。
由 `test_xtquant_minute_uses_max_of_both_overrides` 失败暴露，已移出为独立分支。

### 验收

- 新增套件 **9 passed**；
- 本线 11 套件 **128 例 → 127 passed / 1 failed**（既有红 `test_pit_filter::test_validator_is_single_chokepoint`）；
- 门禁/失败/质量关键词全域 **295 passed + 1 xfailed**（无回归）；
- **核心守护**：`test_threshold_change_does_not_alter_row_level_rejection` 断言
  客户实测异常 bar（`amount/(close×vol)≈8.97`）在新门禁下**仍被 UnitCheck 全拒**，
  仅批次不再判负 ⇒ 行级判拒语义零变更。
...
**云端 etf_minutes close 口径与客户端还原链自洽性疑问** —— 已登记
`docs/pipeline-tech-debt.md`（状态 `REGISTERED`，低优先、与 qfq 域尾巴同池）。
本轮实证揭示口径疑点但未闭环（缺可独立核验的外部基准），解除条件已写明；
不阻塞本批推送。
...
T1/T2/T4 + 问题 3 门禁（`c62c1cc`）并入推送协调批（用户 2026-09-23 指令）。
客户交付文案随推送⑤呈批，含：`git pull` 更新步骤、pyarrow 已进 `.[all]`（无需手动装）、
隔离区归档指引、以及问题 3 的预期行为变化说明。