...
数据面热循环先行（bar 迭代骨架 + bar_prices）→ 撮合内核 → 账户状态/待决队列。**每段过逐位门才进下段。**
...
- 副本将独占 GitHub 公共仓库 `QuantStudioHybrid`（用户指示）
- remote 添加待用户提供完整 URL（含 owner）；添加命令：`git remote add github <URL>`
- **公开风险提示（已向用户提示，公开与否为用户决策）**：仓库含 31 个策略源码——量化策略公开存在 alpha 衰减/被复现风险；浅克隆单提交历史推送轻量（无历史大 blob），建议推送前用户自查工作树内容
- 推送分支：feat/rust-hybrid
