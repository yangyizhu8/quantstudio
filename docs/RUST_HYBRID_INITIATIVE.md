...
## 8. 六步流程状态板（2026-09-21 终止裁决后更新）

| 步 | 内容 | 状态 |
|---|---|---|
| 前置 C | 副本构建 + 交接工件 | ✅ 完成（2026-09-20，29b2dbb） |
| ① | 方案件起草（六节俱全） | ✅ 完成（v1.0→v1.3；72e8c6e） |
| ② | 总调度审 | ✅ 通过（附硬条件三项，已落档） |
| ③ | 实施（分段过门） | ✅ **完成·两段**（M0 对比器 bb80115 / M1 聚合 e63dab7 / M2-P 向量化 ebecd96；**路线 R 终止**——见变更日志终止裁决） |
| ④ | 用户确认 | 🔄 并回包呈审中（MERGEBACK_MANIFEST.md） |
| ⑤ | 并回主仓 | ⏳ 准备完毕（终态锚=收尾 commit；⑤总调度辖，另令执行） |
| ⑥ | 双推 + 同步门 | ⏳ 主仓侧流程（触及共享层→QuantStudio-trading 同步门适用） |
...
| 2026-09-21 | **终止裁决归档（指令③）**：**裁定 = 路线 R（Rust 下沉）终止**；**触发条件** = 路线 P 判定门双口径越线（U2 真口径 2.50×=487.4→194.8s 门禁口径 / 2.81×=三跑中位 173.2s，B 段 80.67%→2.49%）触发审核要点原文条款"P 达 ≥2× → Rust 下沉性能理由即消失，仅剩架构一致性理由，须重新呈裁"；**决策链** = 审核席建议（性能理由消失 / 装载残余非 Rust 独占、Python/duckdb 域可承接 / U2 绝对值 173s 边际收益与必要性显著下降）→ 总调度裁定；**日期 2026-09-21 · 决策人：总调度** | 总调度（终止令） |
