...
| 时间（交易日） | 自动执行的内容 |
|---|---|
| 06:00 | 数据采集守护进程启动（需要开启GUI界面的进场常驻增量拉取，这是用户唯一需要做的，开启后不用管，每天6：00只要电脑不关机，进程会自动拉取） |
...
### Q1：某天数据没有更新？
**A**：先贴「提示词 1」让智能体检查。常见原因：网络中断、MCP 服务临时不可用、数据源晚发布；**少数情况是采集进程非正常结束留下了锁文件**——特征是日志反复出现「写锁被持有」，此时**不会**自动补齐，需按下方「锁残留处置」四步处理（留证据 → 确认进程号已不存在 → 把锁文件**改名**而不是删除 → 重启采集程序）。新版本已具备「残留锁自愈」能力：进程确实已终止后，系统会在 10 分钟内自动回收残留锁并恢复写入，无需人工介入。

#### 锁残留处置（4 步，按顺序执行）

1. **留证据**：把 `<安装目录>\data\snapshots\.write_lock` 与 `data\daemon_status.json` 复制到桌面留存；
2. **确认持有进程已不存在**：用第 1 步文件里看到的进程号执行 `Get-Process -Id <pid> -ErrorAction SilentlyContinue`——**没有任何输出**才可继续；**有输出说明进程仍在运行，请立即停止操作并联系我们**；
3. **把锁文件改名（不要删除）**：`Rename-Item "<安装目录>\data\snapshots\.write_lock" ".write_lock.stale.bak_<日期>"`；
4. **重启采集程序**，观察一轮：日志应出现 `raw=... written=...` 的成功行，且不再出现「写锁被持有」。

#### 怎么判断是哪种情况（避免误操作）

| 日志表现 | 含义 | 您该做什么 |
|---|---|---|
| 「写锁被持有 … **持有者进程已不存在**」 | 进程已终止，残留锁待回收 | 新版本会在约 10 分钟内自动回收；**超过 10 分钟仍未恢复 → 联系我们** |
| 「写锁被持有 … **持有者进程仍存活**」 | 写入进程仍在运行（可能卡住） | 先看该进程状态并回传（`Get-Process -Id <pid>` / macOS `ps -p <pid>`）；**不要**删锁、**不要**强杀 |
| 「无法判定持有者存活」 | 锁文件异常（内容损坏或无进程号） | 回传日志与锁文件，由我们判断 |

> 提醒：直接**删除**或**强行结束**采集进程都可能造成新的残留锁；请始终按上述顺序操作，并在执行前联系技术支持确认。
