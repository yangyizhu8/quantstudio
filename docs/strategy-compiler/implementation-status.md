...
| PR6 IR/renderers/validators | IN_PROGRESS — G1+G2 MERGED TO MAIN | PR6a + PR6b-1 已在 `main`；G1-I `bcdc85d` + G2 CP3 Hermetic Partial Closure `53d90f5` 均已 `--ff-only` 合并到 `main`（当前 main HEAD=`6af7247`；G2 closure commit `53d90f5` 是 main 祖先；G2 delta 不含 G1 engine/API/test 文件）。G2 = Hermetic/Synthetic Reference Partial Closure，input_data_digest=null/data_digest_status=blocked，真实数据 digest/Fidelity 后置；G3/G4 未授权。CP3 Oracle reference `8931430` 保留为独立 truth source。 |
