...
| PR6 IR/renderers/validators | IN_PROGRESS — G1+G2 MERGED TO MAIN; G3 Package Closure IN REVIEW | PR6a + PR6b-1 已在 `main`；G1-I `bcdc85d` + G2 CP3 Hermetic Partial Closure `53d90f5` 均为 `main` 祖先（G2 delta 不含 G1 engine/API/test 文件）。G3 Package Closure 在 `codex/g3-package` 交付 corrective（manifest 可审计性 + G2 linkage 可移植性修复完成，待集中式 Final Review），未合并 main。G2 = Hermetic Reference Partial Closure，input_data_digest=null/data_digest_status=blocked，真实数据 digest/Fidelity 后置；G4 未授权。阶段身份以 commit ancestry 标识（bcdc85d=G1-I、53d90f5=G2 closure 均为 main 祖先），不用会自我失效的固定 main HEAD 字符串。CP3 Oracle reference `8931430` 保留为独立 truth source。 |
