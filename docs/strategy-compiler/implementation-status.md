...
- Contract upgrades: Skill release `0.6.0-runtime-shape-capital-gates`; agent design `2.2`; PTrade profile `1.8.0`; user backtest evidence `2.0`; validation report `2.1`. Design 2.1 artifacts are not auto-migrated and must be regenerated with a fresh R2.5 confirmation.
- Profile 1.8.0 records the `get_history(is_dict=True)` return contract (DataFrame / structured array / recarray items; Series / ndarray fields). The agent-first validator BLOCKs unguarded pandas-only access (`PTRADE-HISTORY-SHAPE-UNSAFE` / `PTRADE-HISTORY-PANDAS-ONLY` / `PTRADE-HISTORY-NORMALIZATION-MISSING`), and `scripts/validate_runtime_shapes.py` executes the strategy's extraction helper against the real shape fixtures (distinct from the legacy renderer fixture).
- Design 2.2 adds machine-checkable `portfolio_contract`, `rebalance_funding_contract`, `history_coverage_contract`, `r5_deployment_invariants`, and verbatim `confirmation_evidence`. Contradictory capital math (20 x 5% + 15% cash buffer) BLOCKs at design time; `runtime_total_value` sizing forbids hardcoded `g.capital`/fixed positive `order_target_value` amounts.
- New `references/execution-funding-matrix.md` freezes current engine funding semantics (close/open immediate, next_open legacy pending, next_open+handle_data+basket atomic); incompatible design combinations BLOCK (`EXECUTION-SELL-PROCEEDS-UNAVAILABLE` etc.). Engine behavior itself is unchanged.
- Evidence 2.0 binds real artifacts (`config.csv`/`daily_stats.csv`/`trades.csv`/run log + SHA-256). `scripts/analyze_backtest_artifacts.py` parses actual capital, match mode, deployment and rejection counters; `review_user_backtest_evidence.py` enforces the capital contract and `r5_deployment_invariants` with new failure classes (`capital_contract_mismatch`, `deployment_invariant_failed`, `execution_funding_failed`, `artifact_missing`, `artifact_hash_mismatch`). Self-reported `runtime_checks` booleans are no longer authoritative.
- R4/R6 reports now distinguish `profile_validation_status` (static) from `runtime_validation_status=NOT_VERIFIED` and `deployment_status=NOT_DEPLOYABLE`; static PASS must never be phrased as deployable.
- New `scripts/retire_ptrade_runtime_evidence.py`: a real broker runtime failure marks old R4 PASS/local evidence/candidate/publish permission STALE in one step and renames old candidate/upload artifacts `*.RETIRED_DO_NOT_UPLOAD`. The old contrarian candidate and `ptrade/contrarian_loser_reversal_ptrade.py` were retired under this protocol and survive only as negative regression fixtures (`tests/fixtures/retired_contrarian/`).
- Regression coverage: `tests/test_agent_ptrade_history_runtime_shapes.py`, `tests/test_agent_portfolio_contract.py`, `tests/test_agent_execution_funding_contract.py`, extended `tests/test_user_pyqt_candidate_flow.py` (capital mismatch / under-deployment / artifact tamper / retirement), plus a Profile 1.8.0 return-contract assertion in `tests/test_ptrade_profile_registered_stock_apis.py`.
- Synchronization status: local repair only until post-repair customer confirmation.

## Agent-first Skill 0.6.0 review hardening (2026-07-27, post-review P0 fixes)

Independent review of the 0.6.0 gates found four P0 bypasses; all are now closed:

- **Runtime-shape fixture is now a hard state-machine gate**, not a manual step. `prepare_user_backtest_candidate.py` runs it whenever a dual-target source consumes `get_history(is_dict=True)` and BLOCKs candidate generation on fixture FAIL; the workspace state records `runtime_shape_fixture_status/source_sha256/report_sha256`; `publish_agent_strategy.py` re-verifies the fixture PASS and the canonical hash (canonical edits make the old fixture PASS STALE). The helper contradiction is resolved by option A: design 2.2 dual strategies using `is_dict=True` must define the standard `_extract_history_field` and route every extraction through it (`PTRADE-HISTORY-HELPER-REQUIRED`).
- **R5 artifact binding bypass closed.** The reviewer now verifies artifact SHA-256 and then analyzes exactly those verified files (`analyze_verified`): CSV paths must equal `result_dir/<canonical name>`, all paths absolute/traversal-free, and `config.csv` must match the declared window, candidate/strategy identity, match mode and `expected_engine_semantics_version` (`ARTIFACT-PATH-MISMATCH` / `ARTIFACT-STRATEGY-MISMATCH` / `ARTIFACT-WINDOW-MISMATCH` / `ARTIFACT-ENGINE-SEMANTICS-MISMATCH`, failure class `artifact_contract_mismatch` → R5). The demonstrated swap (bad run hash-bound, good run analyzed) now FAILs.
- **Basket funding requires proof of activation.** `basket_atomic_sell_first` additionally requires `engine_profile.profile_id='daily-bar-v1'` and `rebalance_mode='callback_basket'` (engine semantics `0.4.0-next_open_basket`), matching the real engine condition; R5 compares `config.csv engine_semantics_version` against the design.
- **Deployment gate is per-rebalance.** `QS_REBALANCE_AUDIT`/`QS_PORTFOLIO_AUDIT` lines are the authoritative rebalance events; each rebalance must show `positions >= ceil(expected x minimum_fill_ratio)`, exposure and cash-ratio thresholds at its own date. Historical max / whole-period averages are no longer accepted. The static audit check is now AST-based (log.* calls with the required keys, reachable from lifecycle/scheduled callbacks) — comments and docstrings cannot satisfy it.
- Legitimate `signal_dependent` no-trade runs may bind `trades_csv=null` with a clean completion log; strict-target designs still require a real trades.csv.
- `retire_artifact` hardening: candidate paths re-validated via `ensure_candidate_path_is_safe`, PTrade artifacts must live under `<project>/ptrade/`, retired files get a content-hash suffix and are never overwritten.
- New regression tests: artifact-swap bypass, gradual-deployment gate, no-trade compatibility (pass + strict block), wrong-strategy artifacts, fixture gates (auto-run in prepare, fixture FAIL block, missing PASS block, stale hash block), design 2.2 helper contract, comment-fooling audit negative.
- Synchronization status: local repair only until post-repair customer confirmation.

## Agent-first Skill 0.6.0 second-review hardening (2026-07-27, residual P0 fixes)

Second-round independent review reproduced two residual bypasses and one audit-closure gap; all closed:

- **Basket semantics declaration is now mandatory, not advisory.** `basket_atomic_sell_first` BLOCKs unless `engine_profile.expected_engine_semantics_version='0.4.0-next_open_basket'` is declared (validator rule plus a schema conditional constraint), so R5 always has an activation value to verify from config.csv. Negative tests: missing field BLOCK, `0.2.0-next_open_pending` BLOCK, and an R5 run whose config shows `0.2.0-next_open_pending` against a basket design fails with `ARTIFACT-ENGINE-SEMANTICS-MISMATCH`.
- **Portfolio-audit association is now rebalance_id one-to-one.** The audit contract gains a mandatory unique `rebalance_id` on both `QS_REBALANCE_AUDIT` and `QS_PORTFOLIO_AUDIT` (static AST validator enforces the key). The reviewer binds each rebalance to exactly one same-id portfolio audit; a portfolio audit can never prove two rebalances, must not predate its rebalance, and must not reach past the next rebalance (next_open may audit on the next trading day via the same id). Duplicate/missing/orphan ids are `evidence_incomplete`. The reproduced bypass — one late portfolio audit silently proving two quarterly rebalances — now FAILs.
