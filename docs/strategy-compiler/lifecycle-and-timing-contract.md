...
2. T 日完整 close 形成信号并以同一 close 成交（`include=True` 即此形态：信号含当前 bar close，同 close 成交 = lookahead 循环）。日线+分钟信号必须 `include=False`（日线仅 T-1 及之前数据；分钟仅前一 bar 及之前数据）；T-day close 仅作为 close 撮合成交价是安全的，因为信号不含当日 close。
