...
4. 2026-09-20 总调度裁定落档：混合副本不上 GitHub（远程待办作废）；增补段门同步纪律——方案定稿与每段实施开始前 `git fetch origin main && git merge origin/main`，行号以同步后为准
...
3. ~~用户提供 GitHub 公共仓库完整 URL 后添加 remote~~（2026-09-20 总调度裁定：作废，本副本不上 GitHub）
4. 段门同步纪律（裁定新增）：方案定稿时与每段实施开始前 `git fetch origin main && git merge origin/main`，行号偏差以同步后状态为准；冲突停下申报
