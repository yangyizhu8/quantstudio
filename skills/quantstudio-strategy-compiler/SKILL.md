...
- A current completed minute may use `get_history(..., frequency='1m', fq='pre', include=True)` only in a confirmed scheduled minute callback with an explicit current-bar cutoff. NOTE (2026-08-13): Minute-frequency include semantics require an engine-level improvement before unification. Current include=False anchors to the previous trading day (not the previous minute bar), which degrades minute strategies; include=True with a current-bar cutoff carries same-bar lookahead risk. Engine improvement (minute include anchors to previous bar) is a separate follow-up task. Minute include=True→PTrade mapping is UNVERIFIED (not yet tested on the PTrade platform) — do not convert minute include semantics until verified.
