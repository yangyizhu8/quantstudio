...
- **E1 原则（2026-09-22 定稿，日线零例外）**：Every `get_history`, `get_history_batch`, and `get_price` call used by generated backtest code must include the literal keyword `fq='pre'` AND `include=False` (both daily and minute frequency). `include=True` is FORBIDDEN in generated backtest code — it leaks the current bar into the signal, creating a lookahead bias. `dypre`, post-adjustment, missing/dynamic values, and `attribute_history` are blocked.
  - E1-1 **通用历史 API 取数口径**：日线（及任何以日线为信号源的）策略信号计算一律经 `get_history` / `get_price` / `get_history_batch` 以 `include=False` 读取前一交易日（D-1）日线——API 恒不返回当前回调所在交易日的日线。该约束由取数口径层强制，不依赖策略作者自觉、不依赖 profile/时点选择；**不存在以「当日已收盘」为理由的例外**。
  - E1-2 **成交价模式由策略语义裁定**：`close` / `open` 均合法；当策略语义为「开盘执行」时必须显式声明 `match_price_mode='open'`——沿用默认 `close` 会使成交价漂移到 D 日收盘，属实质执行价差偏差。
  - E1-3 **`data[code]` 当日 raw 快照仅限执行层判断**（涨跌停/停牌），**不得作为信号输入**；信号价格一律来自 `get_history(..., fq='pre', include=False)`。
  - E1-4 **专门 API 边界**：`get_index_day_bar` 等 profile-aware 专门 API 保留其已审定契约（daily-bar-v1 含当前日 D；恐慌抄底先例），不因 E1 变更——契约变更须走框架流程。凡成交时点早于当日收盘的策略（如 match open），消费该 API 时必须**显式取 D-1 行**（`count>=2` 取 `[-2]`，或断言末行 `trade_date==D` 后弃用）：D 日开盘成交而信号消费 D 日收盘指数行，构成真实未来函数泄漏。
  - E1-5 **实证依据（2026-09-22 四格实测）**：执行日 15:00 / 09:31 × `include` True/False 四格实测证实 `include=False` 锚定 `prev_date`、`include=True` 在执行日 09:31 语境含当日全日 bar（真实泄漏）。实测环境为 proxy profile；include 锚定结论对 profile 通用——锚定由 `attach_bar` 直写 `_prev_date` 实现（ptrade_api.py:598），与 profile 选择无关。
  - E1-6 校验器 `NO-LOOKAHEAD-INCLUDE`（`validate_agent_strategy.py`）已实现 E1-1 的 BLOCK；本原则跨会话/跨智能体/跨项目长期有效。
...
- **平台差异吸收（2026-08-22，PTrade 平台对齐治理 v4，必须遵守）**：市价单单笔上限（创业板/科创板 50,000 股）、交易日历全量/格式混用、`listed_date` 格式差异、`filter_stock_by_status('ST')` 退市风险兜底（P-D9：本地剔 close<1/circ_mv<5 亿，平台仅官方 ST 标记——转换侧 `_QS_FILTER_STATUS_EXT` 注入补齐）均由**框架与转换管线吸收**（source_import 注入拆单/归一化/池过滤），策略层**零平台知识**。**`current_price` / `get_current_data` 不是真实 PTrade API**（官方文档全文核实 + 真实平台模块加载期 NameError，2026-08-22 双证）——双端/PTrade 目标策略**禁止调用**（profile `local_only_symbols` 登记，Validator `TARGET-LOCAL-EXTENSION-BAN` BLOCK）；平台官方取当前价方式 = `data[code].price`（当前周期最新价）/ `get_history` 最新 bar / `get_price`（`get_snapshot` 仅交易场景，回测不可用）。**禁止手写平台兜底**——`def _normalize_date_str`、`def _current_raw_price`、`def _is_delisting_risk`（P-D9 池过滤兜底，模板已注入同名函数）、`g.last_close` 自维护赋值均为校验器 BLOCK（`PTRADE-PLATFORM-FALLBACK-BAN`，函数定义级防误伤）；禁止发现即修，新平台差异一律进 D4 登记序列排队。
