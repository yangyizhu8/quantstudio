...
- Daily-frequency `get_history`/`get_history_batch` calls in generated backtest code must ALSO include the literal keyword `include=False` (signal uses T-1 and earlier completed bars only). `include=True` is FORBIDDEN in daily-frequency backtest code — it leaks the current bar into the signal, creating a lookahead bias (signal contains T-day close while execution occurs at that same T-day close in close mode = circular). Rationale (verified 2026-08-13): strict PIT semantics require signal computation using only completed bars (T-1 and earlier); execution at T-day close is the only price known within the T-day time slice. PTrade platform confirmed 2026-08-13: PTrade include=True returns to T-1 (previous trading day), equivalent to local include=False — so local include=False converts to PTrade include=True.
...
- A current completed minute may use `get_history(..., frequency='1m', fq='pre', include=True)` only in a confirmed scheduled minute callback with an explicit current-bar cutoff. NOTE (2026-08-13): Minute include semantics now verified on both sides. PTrade minute include=True returns the current bar, include=False returns the previous bar — identical to local semantics after the engine fix (minute include=False anchors to the current day, excluding the current bar; ptrade_api.py 2026-08-13). Minute include=True still carries same-bar lookahead risk when the current bar's close is used as signal AND as close-mode execution price — prefer include=False (previous bar signal) + close execution. PTrade conversion: minute include is NOT remapped (NORM-INCLUDE-PTRADE maps daily frequency only).
